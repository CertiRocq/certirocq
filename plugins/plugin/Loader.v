...
Ltac eval_certirocq c := certirocq_eval --allow-unsafe-erasure (let x := c in x) (fun v => exact v).
Notation "'CertiEval' c" := ltac:(eval_certirocq c) (at level 0, c at level 200, only parsing).

Ltac eval_certirocq_typed c := certirocq_eval --allow-unsafe-erasure --typed-erasure (let x := c in x) (fun v => exact v).
