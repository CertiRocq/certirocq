...
Section Refinement.
  ...
  Let src_diverge := @CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.diverge nat Hf_src Ht_src Σ box_dc.
  Let src_diverge_not_stuck := @CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.diverge_not_stuck nat Hf_src Ht_src Σ box_dc.
  Let cmap_consistent' :=
  ...
  Fixpoint value_ref' (v1 : CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value) (v2 : val) : Prop :=
    ...
    in
    match v1, v2 with
    | CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.Con_v c1 vs1, Vconstr c2 vs2 =>
      dcon_to_tag default_tag c1 tgm = c2 /\ Forall2_aux vs1 vs2
    | CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.Clos_v _ _ _, Vfun _ _ _ => True
    | CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.ClosFix_v _ _ _, Vfun _ _ _ => True
    | _, _ => False
    end.

  Definition value_ref (v1 : CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value) (v2 : val) : Prop :=
    match v1, v2 with
    | CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.Con_v c1 vs1, Vconstr c2 vs2 =>
  ...
  Definition refines (M : nat) (e_src : EAst.term) (e_tgt : exp) : Prop :=
    forall (v_src : CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value) (f_src t_src : nat),
  ...
  Proof.
    revert v2 v3.
    induction v1 using CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value_ind'; intros v2 v3 Hval Hpre; inv Hval.
...
Section ComputationalRefinement.
  ...
  Context (Hglob_term :
    forall k decl body,
      declared_constant Σ k decl ->
      decl.(EAst.cst_body) = Some body ->
      exists src_v f t, @CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.eval_env_fuel nat
