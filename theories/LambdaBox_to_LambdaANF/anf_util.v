...
(* ================================================================= *)
  ...
  Definition anf_env_rel' (P : CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value -> val -> Prop)
             (vn : list var) (vs : list CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value) (rho : M.t val) :=
  ...
  Definition global_env_rel' (val_rel : CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value -> val -> Prop)
    ...
    forall k v,
      ...
      exists decl body anf_v,
        ...
        (forall src_v f t,
           @CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.eval_env_fuel _ Hf_src Ht_src Σ box_dc [] body (CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.Val src_v) f t ->
           val_rel src_v anf_v).
  ...
  Definition env_consistent (vn : list var) (rho : list CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value) : Prop :=
    forall i j x,
  ...
  Definition cmap_consistent (vn : list var) (rho : list CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value) : Prop :=
    forall i x k decl body,
      ...
      exists v_i f t,
        ...
        @CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.eval_env_fuel _ Hf_src Ht_src Σ box_dc [] body (CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.Val v_i) f t.
  ...
  Definition cmap_eval_coherent : Prop :=
    forall k1 k2 x decl1 body1 decl2 body2 src_v f t,
      ...
      @CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.eval_env_fuel _ Hf_src Ht_src Σ box_dc [] body1 (CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.Val src_v) f t ->
      exists f' t',
        @CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.eval_env_fuel _ Hf_src Ht_src Σ box_dc [] body2 (CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.Val src_v) f' t'.

  Inductive anf_val_rel : CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value -> val -> Prop :=
  | anf_rel_Con :
  ...
  Definition anf_env_rel : list var -> list CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value -> M.t val -> Prop :=
...
(* ================================================================= *)
  ...
  Lemma NoDup_env_consistent (vn : list var) (rho : list CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value) :
...
Section GlobalCMapFacts.
  ...
  Lemma in_map_fst_nodup_lookup_const_exists cm :
    NoDup (map fst cm) ->
    forall k,
      List.In k (map fst cm) ->
      exists v, lookup_const cm k = Some v.
  Proof.
    induction cm as [| [k' v'] cm' IH]; intros Hnd k Hin.
    - contradiction.
    - inversion Hnd as [| ? ? Hnotin Hnd']; subst.
      simpl in Hin. destruct Hin as [Heq | Hin].
      + subst k'. exists v'. simpl. rewrite eq_kername_refl. reflexivity.
      + destruct (IH Hnd' _ Hin) as [v Hlk].
        exists v. simpl.
        destruct (eq_kername k k') eqn:Hkk'.
        * apply eq_kername_bool_eq in Hkk'. subst k'.
  ...
  Lemma anf_cvt_rel_global_nodup_keys :
    forall S gd cm_acc cm C_env S' Σ_proc,
      anf_cvt_rel_global func_tag default_tag tgm S gd cm_acc cm C_env S' ->
      Σ = List.app (List.rev gd) Σ_proc ->
      wf_glob Σ_proc ->
  ...
  Proof.
    ...
    induction Hcvt as
      ...
      | S0 S0' k gd' cm0 cm' C_rest Hrest IH
      | S0 S0' k ind gd' cm0 cm' C_rest Hrest IH ];
      intros Σ_proc HΣ_eq Hwf_proc Hnd_acc Hcm_sound_proc.
    - exact Hnd_acc.
    - simpl in HΣ_eq.
      rewrite <- List.app_assoc in HΣ_eq.
      simpl in HΣ_eq.
      assert (Hwf_split :
        wf_glob (List.app (List.rev gd')
      ...
      { constructor.
        - intro Hin.
          destruct (in_map_fst_nodup_lookup_const_exists cm0 Hnd_acc k Hin) as [v0 Hlk0].
          destruct (Hcm_sound_proc _ _ Hlk0) as [decl [body0 [Hdecl0 _]]].
          apply Hnotin_proc.
          eapply declared_constant_in_map_fst; [exact Hwf_proc | exact Hdecl0].
        - exact Hnd_acc. }
      assert (Hcm_sound_proc1 :
        forall k0 v0,
          lookup_const ((k, v) :: cm0) k0 = Some v0 ->
          exists decl body0,
            declared_constant
      ...
      { intros k0 v0 Hlk0.
        simpl in Hlk0.
        destruct (eq_kername k0 k) eqn:Hk0k.
        - apply eq_kername_bool_eq in Hk0k. subst k0.
          ...
          + reflexivity.
        - destruct (Hcm_sound_proc _ _ Hlk0) as [decl [body0 [Hdecl0 Hbody0]]].
          exists decl, body0. split.
    ...
    - simpl in HΣ_eq.
      ...
      { rewrite <- HΣ_eq. exact Hwf_glob. }
      assert (Hwf_proc1 :
        wf_glob ((k, EAst.InductiveDecl ind) :: Σ_proc)).
      { eapply suffix_wf with (prefix := List.rev gd'). exact Hwf_split. }
      assert (Hnotin_proc : ~ List.In k (map fst Σ_proc)).
      { eapply key_not_in_suffix with
          (prefix := List.rev gd')
  ...
  Corollary anf_cvt_rel_global_nodup_keys_top :
    forall S cm C_env S',
      anf_cvt_rel_global func_tag default_tag tgm S (List.rev Σ) [] cm C_env S' ->
      NoDup (map fst cm).
  Proof.
    intros S cm C_env S' Hcvt.
    eapply anf_cvt_rel_global_nodup_keys with (Σ_proc := []); eauto.
    - rewrite rev_involutive. rewrite app_nil_r. reflexivity.
    - constructor.
    - constructor.
...
(* ----------------------------------------------------------------- *)
  ...
  Proof.
    ...
    set (P := fun (v : CertiRocq.LambdaBox_to_LambdaANF.fuel_sem.value) =>
      forall v1 v2 : val,
        anf_val_rel' v v1 -> anf_val_rel' v v2 ->
        preord_val cenv PG k v1 v2).
